...
# multi-layer support
class MEHGNN_nc_layer(nn.Module):
    def __init__(self,
                 num_metapaths_list,
                 num_edge_type,
                 etypes_lists,
                 ...
                 rnn_type='gru',
                 attn_drop=0.5):
        super(MEHGNN_nc_layer, self).__init__()
        self.in_dim = in_dim
        ...
        for i in range(len(num_metapaths_list)):
            self.ctr_ntype_layers.append(MEHGNN_ctr_ntype_specific(num_metapaths_list[i],
                                                                  etypes_lists[i],
                                                                  in_dim,
                                                                  num_heads,
                                                                  attn_vec_dim,
...
class MEHGNN_nc_AC(nn.Module):
    def __init__(self,
                 ...
                 is_cuda=False,
                 feat_opt=[]):
        super(MEHGNN_nc_AC, self).__init__()
        ...
        for l in range(num_layers - 1):
            self.layers.append(MEHGNN_nc_layer(num_metapaths_list, num_edge_type, etypes_lists, hidden_dim, hidden_dim,
                                              num_heads, attn_vec_dim, rnn_type, attn_drop=dropout_rate))
        # output projection layer
        self.layers.append(MEHGNN_nc_layer(num_metapaths_list, num_edge_type, etypes_lists, hidden_dim, out_dim,
                                          num_heads, attn_vec_dim, rnn_type, attn_drop=dropout_rate))
